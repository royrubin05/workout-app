...
for move in BASE_MOVEMENTS:
    for eq in move['equipment']:
        ex_id = f"ex-{id_counter}"
        name = f"{eq} {move['name']}"
        # Escape single quotes in names if any (none currently, but good practice)
        name = name.replace("'", "''")
        
        values.append(f"('{ex_id}', '{name}', '{eq}', '{move['category']}', '{move['muscles']}')")
        id_counter += 1

sql += ",\n".join(values) + ";"

with open('migrate_exercises.sql', 'w') as f:
    f.write(sql)

print("Migration file generated: migrate_exercises.sql")
